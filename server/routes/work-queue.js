...
const consentSchema = z.object({
  channels: z
    ...
    .refine(
      (channels) =>
        new Set(channels.map((item) => item.channel)).size === channels.length,
      "Each consent channel may be recorded only once per request.",
    ),
});
...
export async function workQueueRoutes(app) {
  ...
  const messagingGuards = [...crmGuards, app.requireModule("messaging")];
  const bookingGuards = [...crmGuards, app.requireModule("booking")];
  ...
  app.get(
    ...
    async (request) => {
      ...
      if (!parsed.success)
        throw errors.badRequest("The work-queue filters are invalid.");
        ...
        async (client) => {
            ...
            SELECT
              ...
              (
                ...
                WHERE workspace_id = c.workspace_id
                  AND contact_id = c.id
                  AND channel_type = CASE WHEN $3 = 'sms' THEN 'phone' ELSE $3 END
          ...
          if (!target.recipient)
            throw errors.conflict(
              `The contact has no ${body.data.channel} destination.`,
            );
          if (target.consent_status !== "granted") {
            throw errors.conflict(
              `${titleCase(body.data.channel)} consent is not recorded as granted.`,
            );
          }
            ...
            : (
                await client.query(
                 ...
                 WHERE workspace_id = $1 AND idempotency_key = $2`,
                  [request.user.workspaceId, idempotencyKey],
                )
              ).rows[0];
          if (created) {
            await client.query(
  ...
  app.post(
    "/api/v1/contacts/:contactId/appointments",
    {
      preHandler: bookingGuards,
    },
    async (request, reply) => {
      const contactId = z.string().uuid().safeParse(request.params.contactId);
      ...
      const result = await withWorkspaceTransaction(
        ...
        `,
          ...
          await client.query(
            `INSERT INTO contact_timeline_events (
             workspace_id, contact_id, event_type, title, body, actor_user_id, metadata
           )
           VALUES (
             $1, $2, 'appointment_requested', 'Appointment requested', $3, $4,
             jsonb_build_object(
               'appointmentId', $5::text,
               'startsAt', $6::text,
               'endsAt', $7::text,
               'timezone', $8::text
             )
